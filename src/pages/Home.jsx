  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20 font-sans selection:bg-blue-200 selection:text-blue-900">
      ...
      <div className="relative min-h-[85vh] lg:min-h-screen w-full group overflow-hidden bg-slate-900">
        ...
        <div className="relative h-full flex flex-col justify-end px-4 sm:px-8 lg:px-16 pb-28 sm:pb-36 lg:pb-40 w-full sm:w-4/5 md:w-3/4 lg:w-1/2 z-10 min-h-[85vh] lg:min-h-screen">
          <div className="space-y-6">
            ...
            <h2 className="text-5xl sm:text-7xl lg:text-[5.5rem] font-black tracking-tighter text-slate-900 drop-shadow-sm leading-[1.05] animate-fade-in-up" style={{animationDelay: '100ms'}}>
              {featuredMovie.title.split(' (')[0]}
            </h2>
            
            {/* Meta Tags */}
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm font-bold text-slate-700 animate-fade-in-up" style={{animationDelay: '150ms'}}>
               <div className="flex items-center gap-1.5 bg-white/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-200/50 shadow-sm">
                  <Star className="w-4 h-4 text-amber-500 fill-current" /> {featuredMovie.rating}
               </div>
               <div className="flex items-center gap-1.5 bg-white/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-200/50 shadow-sm">
                  <Calendar className="w-4 h-4 text-blue-500" /> {featuredMovie.year}
               </div>
               <div className="flex items-center gap-1.5 bg-white/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-200/50 shadow-sm">
                  <Clock className="w-4 h-4 text-blue-500" /> {featuredMovie.duration}
               </div>
               <span className="bg-slate-900 text-white px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wider font-extrabold shadow-sm">HD</span>
            </div>

            <p className="text-base lg:text-lg text-slate-600 max-w-xl font-medium leading-relaxed animate-fade-in-up line-clamp-3 md:line-clamp-4" style={{animationDelay: '200ms'}}>
              {featuredMovie.desc}
            </p>
      ...
      <div className="relative z-20 px-4 sm:px-8 lg:px-16 space-y-16 sm:space-y-24 -mt-16 sm:-mt-24">
            ...
            {suggestions.map((movie, index) => (
              ...
              >
                ...
                <div className="px-2 transition-transform duration-300 group-hover:translate-x-1">
                  <h4 className="text-xl sm:text-2xl font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors drop-shadow-sm tracking-tight line-clamp-1">{movie.title.split(' (')[0]}</h4>
        ...
        <MovieRow title="Phim Hoạt Hình" indicatorColor="bg-amber-500" movies={animatedMovies} navigate={navigate} categorySlug="hoat-hinh" />
      </div>
    </div>
  );
};
    ...
    <section className="space-y-6 pt-2">
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-2 h-8 rounded-full transform group-hover:scale-y-125 transition-transform duration-300 ${indicatorColor} shadow-sm`}></div>
          <h3 className="text-2xl sm:text-3xl md:text-3xl font-black text-slate-900 group-hover:text-blue-600 transition-colors tracking-tight">{title}</h3>
        </div>
            ...
            <div className="relative aspect-[2/3] rounded-3xl overflow-hidden bg-white shadow-sm hover:shadow-2xl hover:shadow-blue-900/15 transition-all duration-500 transform group-hover:-translate-y-2 ring-1 ring-black/5 group-hover:ring-blue-500/30">
              ...
              <div className="absolute top-3 left-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-y-2 group-hover:translate-y-0">
                 <span className="bg-amber-500/90 backdrop-blur-sm text-white font-extrabold text-[11px] px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1 border border-white/20">
                    <Star className="w-3 h-3 text-white fill-current"/> {movie.rating} 
                 </span>
              </div>
            </div>
            
            <div className="px-1 text-center sm:text-left transition-transform duration-300 group-hover:translate-x-1">
              <h4 className="font-extrabold text-sm md:text-base text-slate-800 group-hover:text-blue-600 transition-colors line-clamp-1">{movie.title.split(' (')[0]}</h4>
