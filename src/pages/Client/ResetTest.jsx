...
import { EyeOff, Film, KeyRound } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0D0D0D' }}>
      <div className="hidden lg:flex lg:w-[52%] relative overflow-hidden flex-col">
        <img src="https://images.unsplash.com/photo-1549490349-8643362247b5?q=80&w=1500&auto=format&fit=crop" alt="" className="absolute inset-0 w-full h-full object-cover opacity-30" />
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="rounded-3xl p-8" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(16px)' }}>
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5" style={{ background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.25)', boxShadow: '0 0 28px rgba(0,212,255,0.15)' }}>
              <KeyRound size={26} style={{ color: '#00D4FF' }} />
            </div>
            <h2 className="text-3xl font-black text-white mb-1">Cập nhật mật khẩu</h2>
            <p className="text-sm mb-7" style={{ color: '#64748B' }}>Nhập mật khẩu mới của bạn</p>
            
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold mb-1.5 block tracking-wider" style={{ color: '#64748B' }}>MẬT KHẨU MỚI</label>
                <div className="relative">
                  <input className={`${inputBase} pr-10`} style={inputNormal} placeholder="••••••••" type="password" />
                  <button type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2" style={{ color: '#64748B' }}>
                    <EyeOff size={15} />
                  </button>
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold mb-1.5 block tracking-wider" style={{ color: '#64748B' }}>NHẬP LẠI MẬT KHẨU</label>
                <div className="relative">
                  <input className={`${inputBase} pr-10`} style={inputNormal} placeholder="••••••••" type="password" />
                  <button type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2" style={{ color: '#64748B' }}>
                    <EyeOff size={15} />
              ...
              <button type="button" className={gradBtn} style={gradStyle}>Đổi mật khẩu</button>
            </div>
            
            <p className="text-center mt-5 text-sm">
              <Link to="/logintest" className="hover:underline" style={{ color: '#00D4FF' }}>Quay lại đăng nhập</Link>
            </p>
          </div>
